...
namespace Poker
{
    public partial class frmPoker : Form
    {
        ...
        private void btnCheck_Click(object sender, EventArgs e)
        {
            ...
            string result = "";
            int multiplier = 0; // 記錄賠率

            // 根據圖片的賠率表設定
            if (isRoyalisFlush)
            {
                result = $"{colorList[0]} 皇家同花順"; // 配合圖片名稱
                multiplier = 250;
            }
            ...
            {
                // 有中獎：贏得的淨利 = 押注金 * 賠率
                netProfit = currentBet * multiplier;
                lblReceive.Text = $"+{netProfit}";
                lblReceive.ForeColor = Color.Red; // 正的時候字顏色為紅
            }
            else
            {
                // 沒中獎 (雜牌，賠率為 0)：純虧損押注金
                ...
                lblReceive.Text = $"{netProfit}";
                lblReceive.ForeColor = Color.Green; // 負的時候字顏色為綠
            }
            ...
            btnChangeCard.Enabled = false;
            btnCheck.Enabled = false;
            btnDealCard.Enabled = false; // 必須重新押注才能再次發牌
